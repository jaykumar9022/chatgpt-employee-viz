...
with open(html_file, "w") as f:
    # Embed verification email in HTML comment
    f.write(f"<!-- {email} -->\n")
    # Embed Python code as preformatted text
    f.write(f"<pre>{python_code}</pre>\n")
    # Embed interactive mpld3 figure
    f.write(mpld3.fig_to_html(plt.gcf()))

print(f"HTML file saved with embedded Python code and interactive plot: {html_file}")
